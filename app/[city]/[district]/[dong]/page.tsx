...
import RandomShopList from "@/components/RandomShopList"; // 하단에 정의된 컴포넌트 활용 또는 별도 분리
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const charSum = (locationKeyword + "gis_clean_dong").split('').reduce((acc, char) => acc + char.charCodeAt(0), 0);
  ...
  const descriptionVariants = [
    ...
    /* 29 */ `${locationKeyword} 최종 웰니스 테라피 제휴 가이드. ${SITE_NAME}가 보증하는 안전하고 쾌적한 휴식 공간 정보.`
  ];
  ...
  return (
    <div className="bg-slate-50 text-slate-800 min-h-screen flex flex-col font-sans pb-16">
      ...
      <main className="max-w-4xl mx-auto px-4 py-8 w-full flex-1 space-y-8">
        ...
        {/* 🌟 새로고침 시 무작위로 순서가 섞이는 추천 제휴 샵 리스트 컴포넌트 */}
...
function RandomShopList({ shops, fullLocation, city, district, dong }: { 
  shops: ShopItem[]; 
  fullLocation: string; 
  city: string; 
  district: string; 
