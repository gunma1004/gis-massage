...
def generate_sitemap():
    base_url = "https://gis-massage.netlify.app"
    ...
    # 4. 전달해주신 최신 regionData 구조 (서울, 경기, 인천 전체 구 및 세부 동 전수 포함)
    regionData = {
        "seoul": {
            "name": "서울특별시",
            "districts": {
        ...
        "incheon": {
            "name": "인천광역시",
            "districts": {
                "jemulpo": {"name": "제물포구", "dongs": ["신포동", "연안동", "신흥동", "도원동", "율목동", "동인천동", "만석동", "화수동", "화평동", "송현동", "송림동", "금창동", "개항동"]},
    ...
    # 5. 제공해주신 최신 regionData 기준 구/시/군 및 세부 동 전체 순회하며 sitemap 생성
    for region, reg_info in regionData.items():
        for district_key, dist_info in reg_info["districts"].items():
            ...
            for dong in dist_info["dongs"]:
                ...
                # 동 하위 샵 상세 페이지 (예: /seoul/종로구/청운동/shop/1)
                for s_id in shop_ids:
                    url_entries.append({
    ...
    print(f"🎉 기인서테라피 최신 지역 데이터 기준 총 {len(url_entries)}개의 URL이 public/sitemap.xml로 생성되었습니다!")
